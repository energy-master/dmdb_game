...
class IdentDecision(RootDecision):
    ...
    def __init__(self, decision_data = None):
        ...
        self.iter = decision_data['iter_cnt']
        
    def __str__(self):
        return ("HP IDent 1.0, Market, {0} , Decision Type, {1} , Memory, {8}, XR_Start, {7}, Time From, {2}, Time To, {6}, Action, {3} , Result, {4}, ID, {5}, iter, {9}".format(self.Market, self.Decision, self.time_bounds[0], self.Decision, self.xr, self.decision_id, self.time_bounds[1], self.xr_start, self.max_memory, self.iter ))
